(function (a, b) {
	...
	var m = {
		dialog: "<div class='modal fade text-left modal' tabindex='-1' role='dialog'><div class='modal-dialog'><div class='modal-content'><div class='modal-body'><div class='bootbox-body'></div></div></div></div></div>",
		header: "<div class='modal-header bg-success white'><h4 class='modal-title white' id='myModalLabel9'><i class='la la-tree'></i></h4></div>",
		footer: "<div class='modal-footer'></div>",
		closeButton: "<button type='button' class='bootbox-close-button close' data-dismiss='modal' aria-hidden='true'><span aria-hidden='true'>×</span></button>",
		form: "<form class='form form-horizontal'></form>",
		inputs: {
	...
	function b(q) {
		...
		k(s, function (v, u, t) {
			...
			if (!u.className) {
				if (r <= 2 && t === r - 1) {
					u.className = "btn-outline-success"
				} else {
					u.className = "btn-outline-danger"
				}
			}
		});
		return q
	...
	h.prompt = function () {
		...
		q.on("submit", function (z) {
			z.preventDefault();
			z.stopPropagation();
			v.find(".btn-outline-success").click()
		});
		v = h.dialog(y);
	...
	h.dialog = function (t) {
		...
		u.on("shown.bs.modal", function () {
			u.find(".btn-outline-success:first").focus()
		});
